...
# Strategy and question analysis
...
def detect_question_topics(question: str) -> set[str]:
    ...
    topic_aliases = {
        ...
        "esg": ["esg", "climate", "sustainability", "stewardship"],
        "high_yield": ["high yield", "credit spread", "junk bond"],
        "emerging_markets": ["emerging market", "embi", "sovereign debt", "em debt"],
        "liquidity": ["liquidity", "liquidity stress", "market functioning"],
        "policy": ["policy", "central bank", "monetary policy", "rate cuts", "rate hikes"],
    }
    found: set[str] = set()
...
def expand_query(question: str, strategy: str | None = None) -> str:
    q = normalize_text(question)
    additions: list[str] = []
...
class MarketChunk:
    def __init__(self, chunk_id, text, metadata, rrf_score, final_score, confidence_score,
                 dense_rank=None, sparse_rank=None, dense_distance=None, sparse_score=None):
...
# Confidence scoring, reranking, and diversity
...
def rerank(fused_chunks, chunk_lookup, question, strategy, top_k):
    ...
    for chunk_id, rrf_score, dense_rank, sparse_rank, dense_dist, sparse_sc in fused_chunks:
        ...
        strategy_text = normalize_text(
            text + " " + str(meta.get("topics", "")) + " " + str(meta.get("source_file", ""))
        )
        ...
        confidence_score = compute_confidence(
            rrf_score=rrf_score, dense_rank=dense_rank, sparse_rank=sparse_rank,
            dense_distance=dense_dist, sparse_score=sparse_sc,
        )

        reranked.append(
            (chunk_id, final_score, confidence_score, rrf_score,
             dense_rank, sparse_rank, dense_dist, sparse_sc)
        )
...
def diversify_chunks(chunks: list[MarketChunk], top_k: int) -> list[MarketChunk]:
    ...
    for chunk in chunks:
        ...
        candidate = MarketChunk(
            ...
            rrf_score=chunk.rrf_score, final_score=adjusted_score,
            confidence_score=chunk.confidence_score, dense_rank=chunk.dense_rank,
            sparse_rank=chunk.sparse_rank, dense_distance=chunk.dense_distance,
...
class MarketIntelAgent:
    def __init__(
        ...
        self.bm25, self.all_ids, self.all_texts, self.all_metadatas = build_bm25_index(
            self.collection, agent_label="Agent 4"
        )
    ...
    def retrieve(self, question, strategy):
        ...
        sparse_chunks = sparse_search(
            ...
            expanded_query, self.top_k_sparse,
        )

        fused = reciprocal_rank_fusion(
            dense_chunks, sparse_chunks, self.top_k_final, k=RRF_K, pool_multiplier=4
        )
        reranked = rerank(
            fused_chunks=fused, chunk_lookup=self.chunk_lookup,
            ...
            if chunk_id in self.chunk_lookup:
                ...
                retrieved.append(
                    MarketChunk(
                        ...
                        confidence_score=confidence_score, dense_rank=dense_rank,
                        sparse_rank=sparse_rank, dense_distance=dense_dist,
    ...
    def run(self, questions, strategy):
        ...
        for i, question in enumerate(questions, 1):
            ...
            question_results.append(
                MarketQuestionResult(question=question, chunks=chunks, expanded_query=expanded_query)
            )
            log("Agent 4", f"Retrieved chunks={len(chunks)} expanded_query={expanded_query}")

        result = MarketIntelResult(strategy=strategy, question_results=question_results)
...
if __name__ == "__main__":
    ...
    result = agent.run(questions=test_questions, strategy="Core Fixed Income")
    print(result)
    print()
